  ...
  return (
    <main className="px-5 py-7 sm:px-7 sm:py-9 xl:px-10">
      <div className="mx-auto max-w-[1280px]">
        <AdminPageHeader eyebrow="Referrals" title="Referral tracking" description="Monitor referred-customer spending and one-time bonuses." />
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {[{ key: "all", label: "All" }, { key: "progressing", label: "Progressing" }, { key: "rewarded", label: "Rewarded" }, { key: "cancelled", label: "Cancelled" }].map((item) => (
            <Link key={item.key} href={item.key === "all" ? "/admin/referrals" : `/admin/referrals?status=${item.key}`} className={`inline-flex h-9 items-center rounded-md border px-3 text-sm font-semibold ${activeStatus === item.key ? "border-black bg-black text-white" : "border-[#c9c9c3] bg-white text-[#686864]"}`}>{item.label}</Link>
          ))}
        </div>
        <section className="mt-5 overflow-hidden rounded-lg border border-[#deded9] bg-white">
